...
pub fn draw_dot(root: &Value, filename: &str) {
    ...
    fn trace(
        ...
        let label = if let Some(ref name) = v.label {
            format!("\"{}|data={:.4}|grad={:.4}\"", name, v.data.borrow(), v.grad.borrow())
        } else {
            format!("\"data={:.4}|grad={:.4}\"", v.data.borrow(), v.grad.borrow())
        };
        
        graph.add_stmt(Stmt::Node(node!(node_id.clone(); attr!("label", label), attr!("shape", "record"))));
        ...
        if let Some(ref op) = v.op {
            ...
            graph.add_stmt(Stmt::Node(node!(op_id.clone(); attr!("label", op_label), attr!("shape", "circle"))));
            graph.add_stmt(Stmt::Edge(edge!(node_id!(op_id.clone()) => node_id!(node_id.clone()))));
            
            for child in children {
                let child_borrowed = child.borrow();
                let child_id = get_node_id(&child_borrowed, node_ids, node_counter);
                graph.add_stmt(Stmt::Edge(edge!(node_id!(child_id.clone()) => node_id!(op_id.clone()))));
                trace(&child_borrowed, graph, visited, node_ids, node_counter);
            }
        }
    }
    
    trace(root, &mut graph, &mut visited, &mut node_ids, &mut node_counter);
    ...
    file.write_all(dot_string.as_bytes()).expect("Unable to write data");
    
    println!("✓ Graph saved to {}", dot_filename);
    println!("  View it at: https://dreampuf.github.io/GraphvizOnline/");
    println!("  Or install Graphviz and run: dot -Tpng {} -o {}.png", dot_filename, filename);
}
